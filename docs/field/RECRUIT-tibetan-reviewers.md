...
请了解：这是一个**学生研究项目**，不是商业产品，未公开发布。项目已私下部署，有 1063 项自动化测试，代码库目前为私有。
...
| 文件 | 行数 | 内容 |
|---|---|---|
| glossary-names.csv | 117 行 | 化验指标名称。每行给出中文、英文原文及释义作参考。例：血红蛋白 / Hemoglobin |
| floor-strings.csv | 161 行 | 界面与安全提示语。例："报告读取服务暂时不可用。请稍后重试。" |
...
**需要翻译的总量：278 行**（117 + 161），均为短句和词条，不是长篇文章。请在 Google Sheets 或 LibreOffice 中工作，导出为 UTF-8 编码的 CSV（Excel 会损坏藏文）。实际工作量约为几次工作时段，不需要数周——但节奏由您掌握，**质量远比速度重要**。
