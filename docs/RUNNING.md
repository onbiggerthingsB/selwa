...
The home page and automated tests run without these credentials. A live call to either
...
Rate limiting is required before Vercel Authentication is removed. It covers both
...
- one shared daily pool of cost units across both paid routes.

At the defaults, an extract costs 5 units, notes translation costs 1 unit, and the
shared pool contains 60 units per day. An IP could therefore make at most 12 extract
requests, 60 notes requests, or a weighted combination, subject to the separate burst
limits.
...
5. Redeploy, then exercise both paid routes and confirm that a limited request returns
...
Operators can change the three primary ceilings directly in Redis:
...
The accepted names are `extract-burst`, `translate-notes-burst`, and `daily-units`.
The value must be a positive integer. A Redis override takes precedence over the
deployment's environment-backed default and is immediately shared by all function
instances. Clear an override to return to the deployed default:
